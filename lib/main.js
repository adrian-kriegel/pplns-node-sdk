...
class HttpClientWrapper {
    ...
    async request(method, url, body) {
        let response;
        try {
            response = await this.client.request(this.buildRequest(method, url, body));
        }
        catch (e) {
            if (e.response) {
                response = e.response;
...
class PipelineApi {
     ...
     * @returns void
     */
    unconsume(taskId, bundleId, consumptionId) {
        return this.client.put('/bundles/' + bundleId + '?' + buildSearchParams({ taskId }), { consumptionId });
    }
}
